...
import { ProjectedPageView } from '../../../../packages/renderer-react/src';
      ...
      <aside className="terminal-sidebar">
        ...
        {activeClock?.phase ? (
          <section className="terminal-block">
            ...
            {activeClock.nowLabel ? <p className="terminal-copy">server/{activeClock.nowLabel}</p> : null}
            {activeClock.nextPhaseLabel ? <p className="terminal-copy">next/{activeClock.nextPhaseLabel}</p> : null}
            {activeClock.source ? <p className="terminal-copy">source/{activeClock.source}</p> : null}
          </section>
        ) : null}
        ...
        {activeWeather?.kind || activeWeather?.patternId ? (
          <section className="terminal-block">
            ...
            {activeWeather.regionId ? <p className="terminal-copy">region/{activeWeather.regionId}</p> : null}
            {activeWeather.patternId ? <p className="terminal-copy">pattern/{activeWeather.patternId}</p> : null}
            {activeWeather.stepId ? <p className="terminal-copy">step/{activeWeather.stepId}</p> : null}
            {activeWeather.source ? <p className="terminal-copy">source/{activeWeather.source}</p> : null}
          </section>
        ) : null}

        {activeAmbientNpcs && activeAmbientNpcs.length > 0 ? (
          <section className="terminal-block">
            ...
            {selectedNodeId ? (
              <>
                ...
                <p className="terminal-copy">node/{selectedNodeId}</p>
              </>
            ) : null}
            <ul className="terminal-list terminal-list--presence">
              {activeAmbientNpcs.map((npc) => {
                const isHere = selectedNodeId !== undefined && npc.nodeId === selectedNodeId;
                const locationLabel = npc.nodeId
                  ? `node/${npc.nodeId}`
                  : `route/${npc.previousNodeId ?? 'unknown'} -> ${npc.nextNodeId ?? 'unknown'}`;

                return (
                  <li key={npc.id} className="terminal-list__item terminal-list__item--presence">
